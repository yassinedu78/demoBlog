...
class ArticleFixtures extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        for($cat = 1; $cat <= 3; $cat++)
        {
            ...
            // mt_rand() : fonction prédéfinie PHP qui retourne un chiffre aléatoire en fonction des arguments transmit, ici un chiffre entre 4 et 10
            for($art = 1; $art <= mt_rand(4,10); $art++)
            {
                ...
                $article->setTitre($faker->sentence())
                        ...
                        ->setCategory($category); // On relie les articles aux catégories déclarées ci-dessus, le setter attend en argument l'objet entité $category pour créer la clé étrangère et non un int
