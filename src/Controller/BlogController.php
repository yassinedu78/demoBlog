...
class BlogController extends AbstractController
{
    ...
    {
        /*
            Injections de dépendances : c'est un des fondement de Symfony, ici notre méthode dépend de la classe ArticleRepository pour fonctionner correctement
            Ici Symfony comprend que la méthode blog() attend en argument un objet issu de la classe ArticleRepository, automatiquement Symfony envoi une instance de cette classe en argument de cette classe
        ...
        */
        ...
        return $this->render('blog/blog.html.twig', [
            'articles' => $articles // on transmet au template les articles selectionnés en BDD afin que twig l'affiche
        ]);
    }
    ...
    public function blogcreate(Article $article = null, Request $request, EntityManagerInterface $manager, SluggerInterface $slugger): Response
    {
        ...
        // Si la variable est null, cela veut dire que nous sommes sur la route '/blog/new', on entre dans le IF et on crée une nouvelle instance de l'entité Article
        ...
        if($formArticle->isSubmitted() && $formArticle->isValid())
        {
            ...
            {
                // Si la photo actuelle est définnit en BDD, alors en cas de modifications, si on ne change pas de photo, on renvoi la photo actuelle en BDD
            ...
            $this->addFlash('success', "L'article a été $txt avec succés !");

            $manager->persist($article);
            $manager->flush();

            // Une fois l'insertion/modification execute en BDD, on redirige l'internaute vers le détail de l'article, on transmet l'id à fournir dans l'url en 2ème paramètre de la mèthode redirectToRoute()
    ...
    # On définir une route 'paramétrée' {id}, ici la route permet de recevoir l'id d'un article stocké en BDD
    #        /blog/5  
    ...
    public function blogShow(Article $article, Request $request, EntityManagerInterface $manager): Response
    {
        ...
        if($formComment->isSubmitted() && $formComment->isValid())
        {
            ...
            $this->addFlash('success', "Bravo votre commentaires à bien été posté !");
